...
from config import MAX_NUMBER_OF_DIFFICULTY, DIFFICULTY_MAP, SLACK_WEB_HOOK_URL
...
def main(event, context):
    ...
    if questions:
        ...
        if message_dict["attachments"]:
            r = requests.post(url=SLACK_WEB_HOOK_URL, json=message_dict)
            if r.text == "ok":
...
def collect_questions():
    questions = []
    older_than_thirty_days = datetime.datetime.utcnow() - datetime.timedelta(days=30)
    for qt_id in range(1, MAX_NUMBER_OF_DIFFICULTY + 1):
        question = Question.objects(
            Q(question_type=qt_id) &
            Q(modified_at__lte=older_than_thirty_days)
        ).first()
...
def frame_message(questions):
    message_dict = {}
    message_dict["text"] = "Here are the Questions of the Week!"
